## JSB
## This snakemake takes each of 7199 isolate samples and aligns them 
...
# 	shell:
# 		"""
# 		module add c3ddb/bowtie2/2.4.2 ;
# 		bowtie2-build -q {input.fasta} {params} 
# 		"""
# rule bowtie2:
# 	input:
# 		fqi=rules.dedupe.output.fqio,
# 		bowtie2idx=ancient(rules.refGenome_index.output.bowtie2idx) # put here, so rule botie2 only executed after rule refGenome_index done
# 	params:
...
# 		"""
# 		module add c3ddb/bowtie2/2.4.2 ;
# 		bowtie2 --very-sensitive --threads 8 -X 2000 --no-mixed --no-unal --dovetail -x {params.refGenome} --interleaved {input.fqi} -S {output.samA} 2> {log} 
# 		"""      
...
#             bamDup = "Mapping/3-bowtie2/{sampleID}_ref_{reference}_aligned_dups.bam",
#             bamDupMate = "Mapping/3-bowtie2/{sampleID}_ref_{reference}_aligned_dups.mates.bam",
#             bamDupMateSort = "Mapping/3-bowtie2/{sampleID}_ref_{reference}_aligned_dups.sorted.mates.bam",
#         output:
#             "Mapping/3-bowtie2/{sampleID}_ref_{reference}_cleanup_done.txt", 
