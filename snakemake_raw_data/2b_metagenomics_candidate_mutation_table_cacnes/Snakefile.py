...
rule all:
    input:
        ...
        #expand("1-temp_pos/{cladeID}_{sampleID}_ref_{reference}_outgroup{outgroup}_positions.mat",sampleID=SAMPLE_ls, reference=REF_Genome_ls, outgroup=OUTGROUP_ls),
        # # Everything # #
...
rule variants2positions:
    ...
    params:
        REF_GENOME_DIRECTORY = "/refgenomedir/{reference}/",
...
rule combine_positions_prep:
    ...
    output:
        string_input_p_positions = "1-temp_pos/{cladeID}_string_file_other_p_to_consider.txt",
    run:
...
rule string_outgroup_bool:
    ...
    output:
      string_outgroup_bool = "1-temp_pos/{cladeID}_string_outgroup_bool.txt",
    run:
      with open( output.string_outgroup_bool ,"w") as f: 
        print(*params.outgroup_bool, sep=" ", file=f)
rule combine_positions:
    input:
        string_input_p_positions = rules.combine_positions_prep.output.string_outgroup_bool,
        string_outgroup_bool = rules.string_outgroup_bool.output.string_outgroup_bool,
    params:
        file_other_p_to_consider = "add_positions/other_positions.mat",
        REF_GENOME_DIRECTORY = expand("/refgenomedir/{reference}/",reference=set(REF_Genome_ls)), # expands to single reference genome!
    output:
        mat_positions = "1-temp_pos/{cladeID}_allpositions.mat",
    shell:
        """
        module add mit/matlab/2015b; 
...
rule string_diversity_mat:
    ...
    output:
        string_diversity_mat = "1-temp_pos/{cladeID}_string_diversity_mat.txt",
    run:
...
rule string_quals_mat:
    ...
    output:
      string_qual_mat = "1-temp_pos/{cladeID}_string_qual_mat.txt",
    run:
...
rule string_sampleID_names:
    params:
        sampleID_names = get_clade_samples,
    output:
        string_sampleID_names = "1-temp_pos/{cladeID}_string_sampleID_names.txt",
    run:
...
rule candidate_mutation_table:
    input:
        mat_positions = rules.combine_positions.output.mat_positions, # "1-temp_pos/{cladeID}_allpositions.mat",
        string_diversity_mat = rules.string_diversity_mat.output.string_diversity_mat, # "1-temp_pos/{cladeID}_string_diversity_mat.txt",
        string_qual_mat = rules.string_quals_mat.output.string_qual_mat, # "1-temp_pos/{cladeID}_string_qual_mat.txt",
        string_sampleID_names = rules.string_sampleID_names.output.string_sampleID_names, # "1-temp_pos/{cladeID}_string_sampleID_names.txt",
        string_outgroup_bool = rules.string_outgroup_bool.output.string_outgroup_bool, # "1-temp_pos/{cladeID}_string_outgroup_bool.txt",
    output:
        candidate_mutation_table = "2-candidate_mutation_table/{reference}/candidate_mutation_table.mat",
    shell:
        """
        module add mit/matlab/2015b; 
