function [f, SimData]=simulate_lineage_differences_subject(SID,A,CutoffAssigned,CutoffReal,DM,T,replacement)
%% remove rows without enough data



enough=sum(A,2,'omitnan')>CutoffAssigned;
A=A(enough,:);
A(A<CutoffReal)=0;
...
%% get numbers of dimentons and incides for permutations
...
%% since unweighted, always equals 1

A=A>0;

%% set middle of DM to NaN to prevent calculation
...
%% get real values for mean, min, max pairwise distance
...
hold on
plot([Dreal Dreal],[0 max(h.Values)],'Color','red','LineWidth',2)
...
% label
xlabel('Average Mean inter-cluster distance')
...
text(h.BinLimits(1),max(h.Values),pv)
% label
xlabel('Average Min inter-cluster distance')
ylabel('Number of simulations')
pbaspect([1 1 1])
...
text(h.BinLimits(1),max(h.Values),pv)
% label
xlabel('Average Max inter-cluster distance')
ylabel('Number of simulations')
pbaspect([1 1 1])
