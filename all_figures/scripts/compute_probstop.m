function probstop = compute_probstop( dir_ref_genome, mut_spec_prob, gene_nums_of_interest )
...
% nonsynonymous
codonstoptable = codon_mutation_table_stop( allmuts, allcodons );
...
probstop = 0; % probability of nonsynonymous mutations over all possible mutations
...
for thismutindex=1:length(allmuts) % loop through all possible mutations
    ...
    % codon
    thismutstopcodon=codonstoptable(:,thismutindex);
    probmutnonstopcodon=thismutstopcodon(thismutcodons);
    
    % Overall probability that this mutation is nonsynonymous over all
    % possible codons
    probmutstop=probthismutation*sum(probmutoncodon.*probmutnonstopcodon);

    % Add contribution of this mutation to the total probability of a nonsynonymous mutation
    probstop = probstop+probmutstop;  
