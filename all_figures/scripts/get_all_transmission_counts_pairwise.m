function [numtransmissions numisolates,Transmissions,Ntotal_genotype_lineage,indices_has_transmissions]=get_all_transmission_counts_pairwise(L,min_subject,min_other)
%%

Transmissions=table;
idx = 1;
indices_has_transmissions=[]
for i =1:size(L,1)
    %     if L.cladenumber(i)~=62
    %         continue
    %     end
    Subjects=char(L.samplenames{i}(~L.outgroup{i}));
    Subjects=string(Subjects(:,1:3));
    TreeSNPs=L.tree_snps{i}';
    %TreeSNPs=L.has_mutation_ingroup_goodpos_no_reco{i}';
...
%%
numtransmissions=vertcat(Transmissions.Ntransmissions{:});
numisolates=vertcat(Transmissions.NisolatesRecipient{:});
Ntotal_genotype_lineage = arrayfun(@(x) sum(~L.outgroup{L.cladenumber==x}), Transmissions.LineageNumber)

histogram(numtransmissions,'BinWidth',1,'BinEdges',0:1:12);
