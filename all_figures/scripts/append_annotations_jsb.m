function  annotations = append_annotations_jsb(annotations, ancnti, calls, cnts, promoterdistance)
...
% 2021.01.25, Arolyn: added semicolon to suppress printing vectors to
% command window (line 78)
...
for i = 1:Npositions
    annotations(i).anc=NTs(ancnti(i)); % Note: ancnti cannot be N
end
...
for i = 1:Npositions
    ...
    elseif (isfield(annotations(i),'WARNING')&string(annotations(i).WARNING)=="scaffold not found")
        annotations(i).type = 'U';
        if ancnti(i) > 0
            annotations(i).nts=[NTs(ancnti(i))];
            if numel(annotations(i).AA)==4
        ...
        if ancnti(i) > 0
            annotations(i).nts=[NTs(ancnti(i))];
            if isfield(annotations(i), "AA")&numel(annotations(i).AA)==4
                annotations(i).AAs(end+1) = annotations(i).AA(ancnti(i));
            end
        else
            annotations(i).nts=char([]);
    ...
    if isfield(annotations(i), "AAs")&numel(annotations(i).AAs)>1
        annotations(i).type='N';
    end

    % Record all observed mutations across all isolates
    % E.g. K134Y, W47A, etc.
    if isfield(annotations(i), "AA")&~isnan(annotations(i).AA) & numel(annotations(i).AA) > 0
        annotations(i).muts={};
