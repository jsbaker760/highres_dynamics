function [treefilename, newnames] = generate_parsimony_tree_jsb(calls, names, prefix_for_labeled_tree)
...
%run
fid = fopen(['temp.sh'],'w');
fprintf(fid, ['#!/bin/sh \n ./dnapars < '  timestamp '_optionfile.txt > '  timestamp '_outfile.txt\n']);
fclose(fid);
...
if exist('prefix_for_labeled_tree','var')
    newtreefilename=char(strjoin([prefix_for_labeled_tree '_' treefilename(1:end-5) '.tree'],''));
else
    newtreefilename=['labeledtree_' treefilename(1:end-5) '.tree'];
end

relabeltree_jsb(treefilename,newtreefilename,tempnames,newnames);
