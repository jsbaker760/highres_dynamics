function [f1 f2 f3] = shared_lineage_directionality(L,n_total,total_shared)
...
f1=figure;hold on;
arrayfun(@(x) plot([x x],[0 dDMRCA(x)],'LineWidth',1,'Color',clrs_arrows(x,:)) ,1:R)
% arrayfun(@(x) annotation([x x],[0 dDMRCA(x)],'LineWidth',1,'Color',clrs_arrows(x,:)) ,1:R)

scatter(1:R,dDMRCA,'filled','CData',clrs_dots)
pbaspect([2,1,1])
xlabel('rank')
ylabel('subject-specific SNPs')
...
has_directionality = numel(unique(L.LineageNumber(dDMRCA>0)));
shared_no_directionality=total_shared-has_directionality;
unshared = n_total-has_directionality - shared_no_directionality;
f2=figure
b=bar(1,[has_directionality shared_no_directionality unshared],'stacked','FaceColor','flat','BarWidth',.5)
b(1).CData=[0 168 117]./255
b(2).CData=[83 192 168]./255
b(3).CData=[.85 .85 .85];

l=legend;
l.String=["has directionality" "unclear direction" " unshared"]
...
% ChildNZ = sum(dDMRCA>0&C<3);
% ChildZ = sum(dDMRCA==0&C<3)
% ParentNZ = sum(dDMRCA>0&C==3)
% ParentZ = sum(dDMRCA==0&C==3)
% % fishertest([ sum(dDMRCA>0&C==3&(ParentSource|ComplexSource)') sum(dDMRCA>0&C==3&(ChildSource)'); sum(dDMRCA==0&C~=3&(ParentSource|ComplexSource)') sum(dDMRCA==0&C~=3&(ChildSource)') ])
% [h,p]=fishertest([  ChildNZ ChildZ; ParentNZ ParentZ])