function [GoodStaphisolate, GoodCacnesIsolate] =  core_genome_filter(CDHITData,AssemblyStats,ncbi_references,SampleNamesAll)
...
%is_Ssaccharolyticus_core = ReferencePcntSepi==0&ReferencePcntScap==0&ReferencePcntSsac==1;
...
[GoodStaphisolate,GoodCacnesIsolate]=deal(false(1,7443));

SampleNamesAllIsolates=string(SampleNamesAll);
idx = arrayfun(@(x) find(x==SampleNamesAllIsolates),string(AssemblyStats.SampleID));

GoodStaphisolate(idx)=SepiGoodBool;
GoodCacnesIsolate(idx)=CacnesGoodBool;
