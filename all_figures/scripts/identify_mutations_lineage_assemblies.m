...
% REFGENOMEDIR is a directory with data for each clade in the format
...
%% get add DNApars to folder where phylogenies are going to be added
...
for k = 1:C
    ...
    n_outgrous_with_call= sum(outgroup_calls>0,2);
    use_outgroup_positions=unambig_outgroup&n_outgrous_with_call>1;
    ...
    %% Make clickable SNP table


%     clickable_snp_table(annotations_full, Calls(goodpos,:), counts(:,goodpos,:), SampleNames_tagoutgroup, ScafNames, MutQual(goodpos), 0);
    ...
    calls_for_tree(Calls_for_analysis>0)=params.NTs(Calls_for_analysis(Calls_for_analysis>0));
    ...
    % make phylogeny
    cd(cluster_tree_dir);
    generate_parsimony_tree_jsb(calls_for_tree, SampleNames_tagoutgroup, subjects_cluster_ingroup);
    cd(workingdir)


    % Directory for tree filtered based off of isoaltes being on different
    % plates
    cluster_tree_dir_filtered = [char(cladedir) '/' 'Tree_filtered'];
    if ~exist(cluster_tree_dir_filtered,'dir')
        mkdir(cluster_tree_dir_filtered)
        copyfile('dnapars', [cluster_tree_dir_filtered '/dnapars'])
        copyfile('dnapars.app', [cluster_tree_dir_filtered '/dnapars.app'])
    end

    [calls_for_tree_filtered, SampleNames_tagoutgroup_filtered,include]=filter_samples_for_strict_tree(calls_for_tree,SampleNames_tagoutgroup);
    % make phylogeny
    cd(cluster_tree_dir_filtered);
    generate_parsimony_tree_jsb(calls_for_tree_filtered, SampleNames_tagoutgroup_filtered, "filtered_tree");
    cd(cluster_tree_dir_filtered)
